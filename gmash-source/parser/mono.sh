...
gmash_parser_mono() {
	...
	while OPTARG= && [ "${GMASH_MONO_ARGR}" != x ] && [ $# -gt 0 ]; do
		...
		case $1 in
			...
			[-+]?*) set "unknown" "$1"; break ;;
			*)
				case $1 in 'subtree'|'remove'|'pull'|'push'|'config'|'clone'|'split'|'list') ;;
					*) set "notcmd" "$1"; break
				esac
				while [ $# -gt 0 ]; do
					GMASH_MONO_ARGR="${GMASH_MONO_ARGR} \"\${$(($OPTIND-$#))}\""
...
Sub-Commands:
  ...
  list                                  List configured subtree metadata.
...
# Generated by getoptions (BEGIN)
# URL: https://github.com/ko1nksm/getoptions (v3.3.2)
export GMASH_MONO_SUBTREE_PREFIX=''
export GMASH_MONO_SUBTREE_REMOTE=''
export GMASH_MONO_SUBTREE_URL=''
export GMASH_MONO_SUBTREE_BR=''
export GMASH_MONO_SUBTREE_SQUASH=''
...
Parameters:
  ...
  -p,     --prefix <subtreePrefixPath>  Subtree prefix path in the monorepo. Prefer not to specify this and let gmash look it up from metadata.
...
Parameters:
  -r,     --remote <subtreeRemote>      Target subtree remote alias.
  -b,     --branch [subtreeBranch]      Target subtree branch. Prefer not to specify this and let gmash look it up from metadata.
  -p,     --prefix [subtreePrefixPath]  Subtree prefix path in the monorepo. Prefer not to specify this and let gmash look it up from metadata.
...
# Generated by getoptions (BEGIN)
# URL: https://github.com/ko1nksm/getoptions (v3.3.2)
export GMASH_MONO_PUSH_PATH=''
export GMASH_MONO_PUSH_REMOTE=''
export GMASH_MONO_PUSH_TGTBR=''
export GMASH_MONO_PUSH_TEMPBR=''
export GMASH_MONO_PUSH_TEMPDIR=''
export GMASH_MONO_PUSH_ALL=''
export GMASH_MONO_PUSH_MAKEPR=''
export GMASH_MONO_PUSH_SQUASH=''
GMASH_MONO_PUSH_ARGR=''
gmash_parser_mono_push() {
	OPTIND=$(($#+1))
	while OPTARG= && [ "${GMASH_MONO_PUSH_ARGR}" != x ] && [ $# -gt 0 ]; do
		set -- "${1%%\=*}" "${1#*\=}" "$@"
		while [ ${#1} -gt 2 ]; do
			case $1 in (*[!a-zA-Z0-9_-]*) break; esac
			case '--path' in
		...
		case $1 in
			--?*=*) OPTARG=$1; shift
				...
				;;
			--no-*|--without-*) unset OPTARG ;;
			-[prBtT]?*) OPTARG=$1; shift
				eval 'set -- "${OPTARG%"${OPTARG#??}"}" "${OPTARG#??}"' ${1+'"$@"'}
				;;
			-[aPshv]?*) OPTARG=$1; shift
				eval 'set -- "${OPTARG%"${OPTARG#??}"}" -"${OPTARG#??}"' ${1+'"$@"'}
				case $2 in --*) set -- "$1" unknown "$2" && GMASH_MONO_PUSH_ARGR=x; esac;OPTARG= ;;
			+*) unset OPTARG ;;
		esac
		case $1 in
			'-p'|'--path')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_PUSH_PATH="$OPTARG"
				shift ;;
			'-r'|'--remote')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_PUSH_REMOTE="$OPTARG"
				shift ;;
			'-B'|'--tgtbr')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_PUSH_TGTBR="$OPTARG"
				shift ;;
			'-t'|'--tempbr')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_PUSH_TEMPBR="$OPTARG"
				shift ;;
			'-T'|'--tempdir')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_PUSH_TEMPDIR="$OPTARG"
				shift ;;
			'-a'|'--all')
				[ "${OPTARG:-}" ] && OPTARG=${OPTARG#*\=} && set "noarg" "$1" && break
				eval '[ ${OPTARG+x} ] &&:' && OPTARG='1' || OPTARG=''
				export GMASH_MONO_PUSH_ALL="$OPTARG"
				;;
			'-P'|'--make-pr')
				[ "${OPTARG:-}" ] && OPTARG=${OPTARG#*\=} && set "noarg" "$1" && break
				eval '[ ${OPTARG+x} ] &&:' && OPTARG='1' || OPTARG=''
				export GMASH_MONO_PUSH_MAKEPR="$OPTARG"
				;;
			'-s'|'--squash')
				[ "${OPTARG:-}" ] && OPTARG=${OPTARG#*\=} && set "noarg" "$1" && break
				eval '[ ${OPTARG+x} ] &&:' && OPTARG='1' || OPTARG=''
				export GMASH_MONO_PUSH_SQUASH="$OPTARG"
				;;
			'-h'|'--help')
				gmash_mono_patch_help
				exit 0 ;;
			'-v'|'--version')
				echo "${GMASH_MONO_PUSH_VERSION}"
				exit 0 ;;
			--)
				shift
				while [ $# -gt 0 ]; do
					GMASH_MONO_PUSH_ARGR="${GMASH_MONO_PUSH_ARGR} \"\${$(($OPTIND-$#))}\""
					shift
				done
				break ;;
			[-+]?*) set "unknown" "$1"; break ;;
...
Parameters:
  ...
  -r,     --remote <subtreeRemote>      Target subtree remote alias.
  -B,     --tgtbr <subtreeBranch>       Target subtree branch.
  -t,     --tempbr <tempBranch>         Name of the temp worktree branch to create for the push operation. (Prefer not to set manually)
  -T,     --tempdir <tempPath>          Path to temp worktree directory to create for the push operation. (Prefer not to set manually)
  -a,     --all                         Patch all known subtrees in the mono repo.
  -P,     --make-pr                     Make a pull request on GitHub with the patched changes.
  -s,     --squash                      Squash strategy when merging subtree changes. Must be consistent with  the previous pull of the subtree.
...
# Generated by getoptions (BEGIN)
# URL: https://github.com/ko1nksm/getoptions (v3.3.2)
export GMASH_MONO_SPLIT_PREFIX=''
export GMASH_MONO_SPLIT_REMOTE=''
export GMASH_MONO_SPLIT_URL=''
export GMASH_MONO_SPLIT_BR=''
export GMASH_MONO_SPLIT_SQUASH=''
export GMASH_MONO_SPLIT_NAME=''
export GMASH_MONO_SPLIT_OWNER=''
GMASH_MONO_SPLIT_ARGR=''
gmash_parser_mono_split() {
	OPTIND=$(($#+1))
	while OPTARG= && [ "${GMASH_MONO_SPLIT_ARGR}" != x ] && [ $# -gt 0 ]; do
		set -- "${1%%\=*}" "${1#*\=}" "$@"
		while [ ${#1} -gt 2 ]; do
			case $1 in (*[!a-zA-Z0-9_-]*) break; esac
			case '--prefix' in
		...
		case $1 in
			...
			-[shv]?*) OPTARG=$1; shift
				eval 'set -- "${OPTARG%"${OPTARG#??}"}" -"${OPTARG#??}"' ${1+'"$@"'}
				case $2 in --*) set -- "$1" unknown "$2" && GMASH_MONO_SPLIT_ARGR=x; esac;OPTARG= ;;
			+*) unset OPTARG ;;
		esac
		case $1 in
			'-p'|'--prefix')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_SPLIT_PREFIX="$OPTARG"
				shift ;;
			'-r'|'--remote')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_SPLIT_REMOTE="$OPTARG"
				shift ;;
			'-l'|'--url')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_SPLIT_URL="$OPTARG"
				shift ;;
			'-b'|'--branch')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_SPLIT_BR="$OPTARG"
				shift ;;
			'-s'|'--squash')
				[ "${OPTARG:-}" ] && OPTARG=${OPTARG#*\=} && set "noarg" "$1" && break
				eval '[ ${OPTARG+x} ] &&:' && OPTARG='1' || OPTARG=''
				export GMASH_MONO_SPLIT_SQUASH="$OPTARG"
				;;
			'-N'|'--name')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_SPLIT_NAME="$OPTARG"
				shift ;;
			'-O'|'--owner')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_MONO_SPLIT_OWNER="$OPTARG"
				shift ;;
			'-h'|'--help')
				gmash_mono_split_help
				exit 0 ;;
			'-v'|'--version')
				echo "${GMASH_MONO_SPLIT_VERSION}"
				exit 0 ;;
			--)
				shift
				while [ $# -gt 0 ]; do
					GMASH_MONO_SPLIT_ARGR="${GMASH_MONO_SPLIT_ARGR} \"\${$(($OPTIND-$#))}\""
					shift
				done
				break ;;
			[-+]?*) set "unknown" "$1"; break ;;
...
Parameters:
  -p,     --prefix <subtreePrefixPath>  Relative path inside the parent repo where the subtree will be split from. Cannot be the root path.
  -r,     --remote <remoteAlias>        Remote alias to add to the parent repo, which will be refered to when pulling and pushing changes to the added subtree.
  -l,     --url <remoteURL>             Remote repository URL of the subtree to add. New github repo won't be created if url is specified.
  -b,     --branch <subtreeBranch>      Target branch of the subtree remote.
  -s,     --squash                      Instead of merging the entire history from the subtree project, produce only a single commit that contains all the differences to merge. Then, merge that new commit into the parent repo. Note, if you add a subtree with --squash, future
 pulls and pushes to that subtree should also be squashed.
  -N,     --name <subtreeRepoName>      Name of the new remote repo to create for the subtree. Required if '--url' is not passed.
  -O,     --owner <subtreeRepoOwner>    Owner (user or org) of the new remote repo to create for the subtree. Required if '--url' is not passed.
...
# Generated by getoptions (BEGIN)
# URL: https://github.com/ko1nksm/getoptions (v3.3.2)
GMASH_MONO_LIST_ARGR=''
gmash_parser_mono_list() {
	OPTIND=$(($#+1))
	while OPTARG= && [ "${GMASH_MONO_LIST_ARGR}" != x ] && [ $# -gt 0 ]; do
		set -- "${1%%\=*}" "${1#*\=}" "$@"
		while [ ${#1} -gt 2 ]; do
			case $1 in (*[!a-zA-Z0-9_-]*) break; esac
			case '--help' in
		...
		case $1 in
			...
			-[hv]?*) OPTARG=$1; shift
				eval 'set -- "${OPTARG%"${OPTARG#??}"}" -"${OPTARG#??}"' ${1+'"$@"'}
				case $2 in --*) set -- "$1" unknown "$2" && GMASH_MONO_LIST_ARGR=x; esac;OPTARG= ;;
			+*) unset OPTARG ;;
		esac
		case $1 in
			'-h'|'--help')
				gmash_mono_list_help
				exit 0 ;;
			'-v'|'--version')
				echo "${GMASH_MONO_SPLIT_VERSION}"
				exit 0 ;;
			--)
				shift
				while [ $# -gt 0 ]; do
					GMASH_MONO_LIST_ARGR="${GMASH_MONO_LIST_ARGR} \"\${$(($OPTIND-$#))}\""
					shift
				done
				break ;;
			[-+]?*) set "unknown" "$1"; break ;;
