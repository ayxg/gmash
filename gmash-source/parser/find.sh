...
export GMASH_FIND_SOURCES_TGTPATH=''
	...
	while OPTARG= && [ "${GMASH_FIND_SOURCES_ARGR}" != x ] && [ $# -gt 0 ]; do
		...
		case $1 in
			...
			'-P'|'--tgt-path')
				[ $# -le 1 ] && set "required" "$1" && break
				OPTARG=$2
				export GMASH_FIND_SOURCES_TGTPATH="$OPTARG"
				shift ;;
			'-h'|'--help')
...
Parameters:
  -p,     --path <path>                 Path to search for source files.
  -P,     --tgt-path <target_path>      Path to merge found source files to.
