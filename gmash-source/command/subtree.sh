...
gmash_subtree_new(){
  ...
  gmash_verb "\e[35m\t⚙ Verifying parameters.\e[0m"
    ...
    if [ "$_correct_branch" != "$_tgtbr" ]; then
      echo "[gmash][new-subtree][error]: Must be on $_tgtbr branch (currently on $_correct_branch)."
      return 1
    fi
  gmash_verb "\e[32m\t  ✓ Params verified, working on mono branch '$_curr_repo_name/$_tgtbr'.\e[0m"

  gmash_verb "\e[34m\tⓘ Final input parameters:\e[0m
    --remote='$_remote',
    --path='$_path',
    --url='$_url',
    --user='$_user',
  ...
  gmash_verb "\e[35m\t⚙ Getting/Creating target subtree repo.\e[0m"
    if [ -z "$_url" ]; then # No url, get an existing remote's url or create a new repo at the given remote.
      ...
      if git ls-remote "https://github.com/$_tgtuser/$_name.git" &> /dev/null; then
          ...
          gmash_verb "\t\t> Found existing GitHub repo '$_tgtuser/$_name.git'. Using it as the subtree remote URL."
      else
        ...
        if git remote get-url "$_remote" >/dev/null 2>&1; then
          ...
          gmash_verb "\t\t> Found existing remote '$_remote' with URL '$_url'. Using it as the subtree remote URL."
          if ! git ls-remote "$_url" &> /dev/null; then
            echo "[gmash][new-subtree][error]: Existing remote '$_remote' URL is not accessible. URL: $_url"
            return 1
          fi
        else
          ...
          if command -v gh >/dev/null 2>&1; then
            gmash_verb "\t\t> Creating new GitHub repo '$_tgtuser/$_name' for subtree..."
            gmash_verb "\e[33m\t\t💻 gh repo create '$_tgtuser/$_name' --private --add-readme --description ...\e[0m"
            if gh repo create "$_tgtuser/$_name" --private --add-readme --description \
              ...
              echo "[gmash][new-subtree][error]: Failed to create GitHub repository."
              return 1
            fi
          else # gh not installed...
              echo "[gmash][new-subtree][error]: GitHub CLI (gh) not found. Please create repository manually."
              return 1
          fi
        fi
      fi
    fi
  gmash_verb "\e[32m\t  ✓ Target subtree repo URL: '$_url' ready for subtree remote '$_remote'.\e[0m"

  gmash_verb "\e[35m\t⚙ Guarding current remotes, subtrees and mono paths. Overwrite disabled.\e[0m"
    if git config "remote.$_remote.url" > /dev/null; then
        echo "[gmash][new-subtree][error]: Remote '$_remote' already exists."
        return 1
    fi
    gmash_verb "\t\t-> Remote '$_remote' is unique."

    if git remote -v | awk '{print $2}' | grep -q "^$_url$"; then
        echo "[gmash][new-subtree][error]: URL '$_url' is already used by another remote."
        return 1
    fi
    gmash_verb "\t\t-> URL '$_url' is not used by any existing remote."

    if [ -e "$_path" ] && [ -n "$(find "$_path" -maxdepth 1 -mindepth 1 -print -quit 2>/dev/null)" ]; then
        echo "[gmash][new-subtree][error]: Target path '$_path' exists and contains files."
        return 1
    fi
    gmash_verb "\t\t-> Target path '$_path' is clear."

    if git check-ignore -q "$_path"; then
        echo "[gmash][new-subtree][error]: Target path '$_path' matches gitignore patterns"
        return 1
    fi
    gmash_verb "\t\t-> Target path '$_path' is not ignored by gitignore."
  gmash_verb "\e[32m\t  ✓ Safe to add subtree.\e[0m"

  # Do all the dirty subtree creation git commands... Im still unsure which parts here are necessary.
    gmash_verb "\e[33m\t💻 git remote add -f '$_remote' '$_url'"
    git remote add -f "$_remote" "$_url"

    gmash_verb "\e[33m\t💻 git subtree add --prefix='$_path' '$_remote' '$_tgtbr'"
    git subtree add --prefix="$_path" "$_url" "$_tgtbr"

    gmash_verb "\e[33m\t💻 git subtree pull --prefix='$_path' '$_remote' '$_tgtbr'"
    git subtree pull --prefix="$_path" "$_remote" "$_tgtbr"

    gmash_verb "\e[33m\t💻 git push"
    git push

    gmash_verb "\e[32m\t  ✓ Subtree '$_remote' added to '$_path' and pushed to remote.\e[0m"
    gmash_verb "\e[34m\t[gmash][new-subtree] ⓘ Result metadata:\e[0m
      \t\t[Remote URL]: '$_url'
