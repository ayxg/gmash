...
gmash_def_parser_main() {
  ...
  msg -- "  "
  msg -- "Globals:"
    flag GMASH_VERBOSE --verbose init:=0 -- "Globally enable verbose output."
    flag GMASH_QUIET   --quiet   init:=0 -- "Globally disable output. !Warning: some command outputs may be suppressed."
    param GMASH_CONFIG --config  var:"<gmashConfigFile>" init:=".gmashuser" -- "Use a gmash argument config file."
  msg -- "  "
  msg -- "Commands:"
    cmd dirs \
  ...
  msg -- "Display:"
    ...
    disp :gmash_versions_index -V --versions -- "Display versions of gmash and all commands/subcommands."
}